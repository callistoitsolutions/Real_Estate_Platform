...
def residential_add_rm(request):
    # 1. Retrieve identity from browser session
 ...
 # ---------- AMENITIES & FACILITIES ----------
            ...
            if user_obj:
                ...
                uploader_role = "Relationship Manager"
    ...
    return render(request, 'rm_panel/Forms/Rental/residential.html', {
        'user_obj': user_obj,
        'user_role': user_role,
        'enquiry_obj_rm':enquiry_obj_rm,
        'ameneties_obj':ameneties_obj,
...
def residential_rm_edit(request, pk):
    ...
    return render(request, 'rm_panel/Forms/Rental/residential_edit.html', context)


########### Views end for rental edit forms for RM ###########################


############### Views start for rental Delete forms for RM ###################



@require_POST
def rental_residential_delete_rm(request, pk):
    """RM Soft Delete Property"""

    try:
        # ======================================
        # RM Session Validation
        # ======================================
        session_user_id = request.session.get('User_id')
        logged_in_role = request.session.get('user_type')

        if not session_user_id or logged_in_role != "Relationship Manager":
            return JsonResponse({
                'status': 'error',
        ...
        # ======================================
        try:
            rm_obj = User_Details.objects.get(id=session_user_id)

            user_identity = (
                f"User ID: {rm_obj.user_id} | "
                f"Name: {rm_obj.user_name} | "
                f"Email: {rm_obj.user_email} | "
                f"Phone: {rm_obj.user_phone}"
            )

            user_role = rm_obj.user_role or "Relationship Manager"
        ...
        # ======================================
        # Security Check
        # RM can delete only own property
        # ======================================
        ...
        # Property Details
        # ======================================
        associated_origin_file = (
            prop.upload_file_name
            if prop.upload_file_name
            else "Web UI Form"
        )

        property_title_ref = (
            prop.property_title
            if prop.property_title
            else "N/A"
        )
...
@require_POST
def rental_bulk_delete_rm(request):
    """RM Advanced Bulk Delete (Soft Delete) — scoped to the RM's own properties only."""
    ...
    try:
        ...
        # ======================================
        # Base Queryset — HARD SCOPED TO THIS RM ONLY
        # CHANGED: was listed_by_id=session_user_id
        # NOW: listed_by_id=rm_obj.user_id
        # ======================================
        properties = RentalResidentialProperty.objects.filter(
            is_deleted=False,
            listed_by_id=rm_obj.user_id
        )
        ...
        deleted_titles = list(
            target_props.values_list('property_title', flat=True)
        )
...
########### Views end for rental delete forms for RM ###########################
...
def residential_resale_rm_list(request):
    ...
    all_properties = (
        ResaleResidentialProperty.objects.filter(listed_by_id=user_obj.user_id,listed_by_name=user_obj.user_name,listed_by_email=user_obj.user_email,listed_by_contact=user_obj.user_phone,listed_by_role=user_obj.user_role)
    ...
    # Show 25 properties per page
    paginator = Paginator(properties, 25) 
    page_number = request.GET.get('page')
    paginated_properties = paginator.get_page(page_number)

    # ── Thumbnail + helper attributes for each filtered property on current page ──
    ...
    context = {
        'user_obj'  : user_obj,
        'enquiry_obj_rm':enquiry_obj_rm,
        'properties' : paginated_properties,  # Pass paginated object to context
...
############## Views start for view resale property form residential ##################

def residential_resale_rm_view(request,id):
 # 1. Retrieve identity from browser session
    session_user_id = request.session.get('User_id')
    logged_in_role = request.session.get('user_type')
...
# Fetch Resale Residential Object via PK
    prop = get_object_or_404(ResaleResidentialProperty, pk=id)
    ...
    return render(request,'rm_panel/Forms/Resale/residential_resale_view.html',context)

############## Views end for view resale property form residential ######################
...
########## Views start for update resale property form commercial ##################

def commercial_resale_rm_update(request,id):
    ...
    return render(request,'rm_panel/Forms/Resale/commercial_resale_edit.html',context)

############ Views end for update resale property form commercial #######################


############ Views start for resale property form commercial ########################

def commercial_resale_rm_view(request,id):
    # 1. Retrieve identity from browser session
    session_user_id = request.session.get('User_id')
    logged_in_role = request.session.get('user_type')
    ...
    return render(request, 'rm_panel/Forms/Resale/commercial_resale_view.html', context)
...
############# Views start for update residential plot list form ###################

def plot_resale_res_rm_update(request,id):
    ...
    return render(request,"rm_panel/Forms/Resale/Plots/update_resi_plots.html",context)

############# Views end for update residential plot list form #######################


############# Views start for view plot residential form #####################

def plot_resale_res_rm_view(request,id):
    ...
    return render(request,"rm_panel/Forms/Resale/Plots/view_resi_plots.html",context)

########### Views end for view plot residential form #############################
...
############# Views start for update resale proerty form industrial ################
...
# 1. Retrieve identity from browser session
    ...
    return render(request, 'rm_panel/Forms/Resale/industrial_edit.html', context)

############ Views end for update resale property form industrial #####################


########### Views start for view resale property form industrial ####################
...
# 1. Retrieve identity from browser session
    ...
    return render(request, 'rm_panel/Forms/Resale/industrial_view.html', context)

############# Views end for view resale property form industrial ####################
...
############ Views start for update agricultural resale proprty form ###################
...
# 1. Retrieve identity from browser session
    ...
    return render(request, 'rm_panel/Forms/Resale/edit_agricultural_resale.html', context)
...
########## Views start for Rental Residential Listing Excel/Download Module for RM #####################
...
def _residential_field_map():
    ...
    sections = OrderedDict([
        ("Listed By", [
            ("listed_by_type",   "Listed By Type (Self)", False),
...
@csrf_exempt
@require_POST
def import_residential_excel_rm(request):
    ...
    # This is the canonical identity every row's Listed By fields must STRICTLY match.
    session_identity = {
        'id': str(rm_obj.user_id).strip(),
        'name': str(rm_obj.user_name).strip(),
        'email': str(rm_obj.user_email).strip().lower(),
        'contact': str(rm_obj.user_phone).strip(),
        'role': str(rm_obj.user_role).strip(),
    }
    ...
    for row_idx, row in enumerate(ws.iter_rows(min_row=data_start_row, values_only=True), start=data_start_row):
        ...
        # ---- DETAILED "Listed By" Field Verification ----
        typed_id = str(obj_data.get('listed_by_id', '')).strip()
        typed_name = str(obj_data.get('listed_by_name', '')).strip()
        typed_email = str(obj_data.get('listed_by_email', '')).strip().lower()
        typed_contact = str(obj_data.get('listed_by_contact', '')).strip()
        typed_role = str(obj_data.get('listed_by_role', '')).strip()

        mismatches = []
        if typed_id != session_identity['id']:
            mismatches.append("ID")
        if typed_name.lower() != session_identity['name'].lower():
            mismatches.append("Name")
        if typed_email != session_identity['email']:
            mismatches.append("Email")
        if typed_contact != session_identity['contact']:
            mismatches.append("Contact")
        if typed_role.lower() != session_identity['role'].lower():
            mismatches.append("Role")

        if mismatches:
            if not any([typed_id, typed_name, typed_email, typed_contact, typed_role]):
                reason = "All 'Listed By' details are completely blank."
            else:
                reason = f"The following 'Listed By' field(s) are missing or incorrect: {', '.join(mismatches)}."

            identity_mismatch_errors.append({
                "row": row_idx,
                "errors": [f"{reason} Row skipped — this upload requires exact matching credentials for all 'Listed By' attributes."],
            })
            skipped_identity_mismatch += 1
            continue

        # Verified match -> lock in canonical session values explicitly and mark Self.
        ...
        if existing_duplicates.exists():
            ...
            if hard_blocked:
                duplicate_blocked_rows.append(
                    ...
                    f"by/for this user. Row skipped; edit the existing listing instead."
                )
                skipped += 1
                continue
...
########## Views End for Rental Residential Listing Excel/Download Module for RM #####################



########## Views start for Rental commercial Listing Module for RM #####################
...
def commercial_rental_add_rm(request):
    user_id = request.session.get('User_id')
    admin_id = request.session.get('Admin_id') 
    logged_in_role = request.session.get('user_type')
    ...
    return render(request, 'rm_panel/Forms/Rental/commercial.html'
   , {
        'user_obj': user_obj,
        'user_role': user_role,
        'enquiry_obj_rm':enquiry_obj_rm,
...
def commercial_rm_edit(request, pk):
    ...
    return render(request, 'rm_panel/Forms/Rental/commercial_edit.html', context)
...
def commercial_view_rm(request, pk):
    user_id = request.session.get('User_id')
    admin_id = request.session.get('Admin_id') 
    logged_in_role = request.session.get('user_type')
    ...
    return render(request, 'rm_panel/Reports/Rental/commercial_detail.html', {
        'enquiry_obj_rm': enquiry_obj_rm,
        'user_obj': user_obj,
        'user_role': user_role,
        'prop': prop,
...
@require_POST
def commercial_bulk_delete_rm(request):
    """RM Advanced Bulk Delete (Soft Delete) — scoped to the RM's own commercial properties only."""
    ...
    try:
        ...
        try:
            ...
            return JsonResponse({'status': 'error', 'message': 'RM not found.'})
        ...
        # ======================================
        # Base Queryset — SCOPED TO THIS RM ONLY
        # ======================================
...
def export_commercial_rent_rm(request):
    session_user_id = request.session.get('User_id')
    logged_in_role = request.session.get('user_type')
...
def _commercial_rental_field_map_rm():
    """
...
def _commercial_sample_row_data_rm():
    """
...
@require_GET
def download_commercial_rental12_template_rm(request):
    """Download the upload template for Commercial Rental Relationship Managers. 
    Listed By identity columns (name/email/contact/role) are MANDATORY and 
    must exactly match your own registered RM profile — a row that's 
    blank or names someone else is skipped on upload. The sample row 
    (row 4) is protected as read-only; data-entry rows (5+) stay fully 
    editable."""

    sections, *_ = _commercial_rental_field_map_rm()
    sample = _commercial_sample_row_data_rm()
    ...
    response["Content-Disposition"] = 'attachment; filename="Commercial_Rental_RM_Template.xlsx"'
    wb.save(response)
    return response
...
@csrf_exempt
@require_POST
def import_commercial_rental_excel_rm(request):
    ...
    (
        sections, field_to_label, label_to_field, system_injected,
        helper_only_labels, decimal_fields, int_fields, float_fields, bool_fields
    ) = _commercial_rental_field_map_rm()
    sample = _commercial_sample_row_data_rm()
    ...
    for r_idx in range(data_start_row, len(rows)):
        ...
        mismatches = []
        if typed_id and typed_id != session_identity['id']:
            mismatches.append("ID")
        if typed_name.lower() != session_identity['name'].lower():
            mismatches.append("Name")
        if typed_email != session_identity['email']:
        ...
        obj_data['listed_by_role'] = session_identity['role']
        obj_data['listed_by_type'] = 'Self'
        obj_data['assigned_to'] = f"{rm_obj.id}-{rm_obj.user_role}"
    ...
    # ---- 6. Write to DB (Fingerprint Duplicate Engine) ----
    created, updated, skipped, errors = (
        0, 0, skipped_empty_after_mapping + skipped_identity_mismatch, []
    )
    duplicate_blocked_rows = []
...
@require_POST
def commercial_delete_rm(request, pk):
    """RM Soft Delete Commercial Property"""
    ...
    try:
        ...
        # ======================================
        # Security Check — RM can delete only own property
        # ======================================
        if str(prop.listed_by_id) != str(rm_obj.user_id):
            return JsonResponse({
                'status': 'error',
...
############ Views end for Rental commercial Listing Module for RM #######################
...
############# Views start for Rental pg Module  for RM ########################
...
def pg_rm_list(request):
    ...
    return render(request, 'rm_panel/Reports/Rental/pg_list.html', {
...
def add_pg_rm(request):
    ...
    is_valid_rm = (
        session_user_id and
        logged_in_role == "Relationship Manager"
    )

    is_valid_admin = (
        logged_in_role == "Admin" and
        'impersonate_id' in request.session
    )
    ...
    try:
        ...
        if logged_in_role == "Admin":
            ...
            admin_obj = Admin_Login.objects.filter(
                id=session_user_id
            ).first()
            ...
            uploader_contact = getattr(admin_obj, 'phone', '') if admin_obj else ''
            uploader_role = getattr(admin_obj, 'role', 'Admin') if admin_obj else 'Admin'
            uploader_id = f"ADMIN_{session_user_id}"

        else:

            rm_obj = User_Details.objects.filter(
                id=dashboard_user_id
            ).first()

            uploader_name = getattr(rm_obj, 'user_name', '') if rm_obj else ''
            uploader_email = getattr(rm_obj, 'user_email', '') if rm_obj else ''
            uploader_contact = getattr(rm_obj, 'user_phone', '') if rm_obj else ''
            uploader_role = getattr(
                rm_obj,
                'user_role',
                'Relationship Manager'
            ) if rm_obj else 'Relationship Manager'

            uploader_id = f"USER_{dashboard_user_id}"
...
def pg_rm_edit(request, pk):
    ...
    if request.method == "GET":
        ...
        return render(request, 'rm_panel/Forms/Rental/pg_edit.html', context)
    ...
    try:
        ...
        return JsonResponse({
            ...
            "redirect_url": reverse('pg_rm_list')
        })
...
def export_pg_coliving_rm(request):
...
def pg_coliving_view_rm(request, pk):
    ...
    return render(request, "rm_panel/Reports/Rental/pg_coliving_view.html", context)





def _pg_coliving_field_map_rm():
    """
    Returns field mappings where the label strings are an EXACT match to the
...
@csrf_exempt
@require_POST
def import_pg_excel_rm(request):
    ...
    (
        sections, field_to_label, label_to_field, system_injected,
        helper_only_labels, int_fields, bool_fields
    ) = _pg_coliving_field_map_rm()

    # -------------------------------------------------------------------
    # REQUIRED FIELDS - Must match the exact keys defined in the dictionary
    # returned by _pg_coliving_field_map(). NOTE: 'listed_by_role' is no
    # longer listed here — it is now enforced by the session identity
    # match step below (same as the Commercial Rental RM import).
    # -------------------------------------------------------------------
    REQUIRED_FIELD_KEYS = [
        'property_no',
        'city',
    ...
    # ---- 2. SECURITY CHECK (mirrors Commercial Rental RM import) ----
    session_user_id = request.session.get('User_id')
    logged_in_role = request.session.get('user_type')
    ...
    # ---- 5. Uploader Identity (system audit trail — who UPLOADED the file) ----
    admin_id = request.session.get('Admin_id')
    admin_obj = Admin_Login.objects.filter(id=admin_id).first() if admin_id else None
    ...
    for r_idx in range(data_start_row, len(rows)):
        ...
        # ---- SESSION-BASED "LISTED BY" IDENTITY VERIFICATION ----
        # (mirrors Commercial Rental RM import — no DB lookup by row content;
        # the row must match the logged-in RM's own session identity exactly.
        # Any blank/mismatched field => row skipped, nothing is written.)
        typed_id = str(obj_data.get('listed_by_id', '')).strip()
        typed_name = str(obj_data.get('listed_by_name', '')).strip()
        typed_email = str(obj_data.get('listed_by_email', '')).strip().lower()
        typed_contact = str(obj_data.get('listed_by_contact', '')).strip()
    ...
    # ---- 7. Write to DB (Fingerprint Duplicate Engine — mirrors Commercial) ----
    created, updated, skipped, errors = 0, 0, skipped_empty_after_mapping + skipped_identity_mismatch, []
    duplicate_blocked_rows = []
...
@require_GET
def download_pg_template_rm(request):
    """Download the upload template for PG/Co-living Relationship Managers.
    Listed By identity columns (name/email/contact/role) are MANDATORY and
    must exactly match your own registered RM profile — a row that's
    blank or names someone else is skipped on upload. The sample row
    (row 4) is protected as read-only; data-entry rows (6+) stay fully
    editable.
    """
    (sections, field_to_label, _, _, _, _, _) = _pg_coliving_field_map_rm()
    ...
    hints = {
        "listed_by_role": "Must match your registered RM role exactly",
        "listed_by_name": "Must match your registered RM name exactly",
        "listed_by_email": "Must match your registered RM email exactly",
        "listed_by_contact": "Must match your registered RM contact exactly",
    ...
    for ci, (fkey, flbl, freq, _) in enumerate(all_cols, 1):
        ...
        # ---- Session-identity comments (mirrors Commercial Rental RM template) ----
        if fkey in ("listed_by_name", "listed_by_email", "listed_by_contact", "listed_by_role"):
            ws.cell(row=2, column=ci).comment = Comment(
                "MANDATORY. Must exactly match your own registered RM profile\n"
                "(name / email / contact / role). Leaving this blank, or entering\n"
                "a different person's details, will cause the row to be SKIPPED\n"
                "on upload and flagged as an alert.",
        ...
        if fkey == "listed_by_id":
            ws.cell(row=2, column=ci).comment = Comment(
                "Optional — your Listed By ID is confirmed from your session once\n"
                "the name/email/contact/role above match your profile.",
                "System"
            )
        if fkey == "listed_by_type":
            ws.cell(row=2, column=ci).comment = Comment(
                "Informational only. Every accepted row is treated as your own\n"
                "(Self) listing once the identity fields are verified.",
                "System"
    ...
    wc = ws.cell(row=5, column=1, value="⚠️ Purple Headers = Mandatory | Row 3 = Hints | Row 4 = SAMPLE DATA (Delete before importing) | Listed By fields must match YOUR registered profile exactly")
    ...
    # ---- Sheet protection: sample row locked, data-entry rows (6+) editable ----
    ...
    resp["Content-Disposition"] = 'attachment; filename="PG_Coliving_Import_Template_rm.xlsx"'
    wb.save(resp)
    return resp


@require_POST
def pg_bulk_delete_rm(request):
    """RM Advanced Bulk Delete (Soft Delete) — scoped to the RM's own PG/Coliving properties only."""
    ...
    try:
        ...
        try:
            ...
            return JsonResponse({'status': 'error', 'message': 'RM not found.'}, status=404)
...
@require_POST
def pg_coliving_delete_rm(request, pk):
    """RM Soft Delete PG/Coliving Property"""
    ...
    try:
        ...
        # Security Check — RM can delete only own property
        # ======================================
        if str(pg.listed_by_id) != str(rm_obj.user_id):
            return JsonResponse({
                'status': 'error',
                'message': 'You can delete only your own properties.'
            })
...
############# Views End for Rental pg Module for RM ########################



############# Views Start for Resale Plot Industrial Module for RM ######################################



def industrial_plot_resale_rm(request):
    session_user_id = request.session.get('User_id')
    logged_in_role = request.session.get('user_type')
    ...
    return render(request, "rm_panel/Forms/Resale_Plot/industrial_plot_resale.html", context)
...
def industrial_plot_resale_add_rm(request):
    ...
    return render(request, 'rm_panel/Reports/Resale_Plot/industrial_plot_resale_list.html', {
...
def industrial_plot_resale_edit_rm(request, pk):
    ...
    "redirect_url": reverse('industrial_plot_resale_list_rm')
})
        except Exception as e:
            print("ERROR DETECTED:", str(e))
            return JsonResponse({
    ...
    return render(request, 'rm_panel/Reports/Resale_Plot/industrial_plot_resale_edit.html', {
...
def industrial_plot_resale_view_rm(request, pk):
    session_user_id = request.session.get('User_id')
    logged_in_role = request.session.get('user_type')
    ...
    return render(request, 'rm_panel/Reports/Resale_Plot/industrial_plot_resale_view.html', {
        'user_obj': user_obj,
        'user_role': user_obj.user_role,
        'enquiry_obj_rm':enquiry_obj_rm,
        'plot': plot,
...
def industrial_plot_resale_list_rm(request):
    ...
    return render(request, "rm_panel/Reports/Resale_Plot/industrial_plot_resale_list.html", context)




@require_POST
def industrial_plot_resale_delete_rm(request, pk):
    """RM Soft Delete Industrial Plot Resale Property"""
    ...
    try:
        ...
        # Security Check — RM can delete only own property
        # ======================================
        if str(plot.listed_by_id) != str(rm_obj.user_id):
            return JsonResponse({
                'status': 'error',
                'message': 'You can delete only your own properties.'
            })
        ...
        IndustrialPlotResaleActivityLog.objects.create(
            ...
            action_type="DELETE",
            property_id=plot.id,
            action_payload=json.dumps({"reason": "Manual delete via RM list page"}),
            status="SUCCESS",
        )
...
@require_POST
def industrial_plot_resale_bulk_delete_rm(request):
    """RM Advanced Bulk Delete (Soft Delete) — scoped to the RM's own industrial plots only."""
    ...
    try:
        ...
        if delete_type == "current_page":
            ...
            pass  # qs already scoped to this RM's non-deleted rows
...
def export_industrial_plot_resale_rm(request):
...
############# Views End for Resale Plot Industrial Module for RM ######################################




############# Views Start for Resale Plot Agriculture Module for RM ######################################



def agricultural_plot_resale_rm(request):
    session_user_id = request.session.get('User_id')
    logged_in_role = request.session.get('user_type')
    ...
    return render(request, "rm_panel/Forms/Resale_Plot/agricultural_plot_resale.html", context)
...
def agricultural_plot_resale_add_rm(request):
    session_user_id = request.session.get('User_id')
    logged_in_role = request.session.get('user_type')
    ...
    return render(request, 'rm_panel/Reports/Resale_Plot/agricultural_plot_resale_list.html', {
...
def agricultural_plot_resale_edit_rm(request, pk):
    session_user_id = request.session.get('User_id')
    logged_in_role = request.session.get('user_type')
    ...
    return render(request, 'rm_panel/Reports/Resale_Plot/agricultural_plot_resale_edit.html', {
...
def export_agricultural_plot_resale_rm(request):
...
def agricultural_plot_resale_view_rm(request, pk):
    ...
    return render(request, 'rm_panel/Reports/Resale_Plot/agricultural_plot_resale_view.html', {
        'user_obj': user_obj,
...
def agricultural_plot_resale_list_rm(request):
    session_user_id = request.session.get('User_id')
    logged_in_role = request.session.get('user_type')
    ...
    return render(request, "rm_panel/Reports/Resale_Plot/agricultural_plot_resale_list.html", context)




@require_POST
def agricultural_plot_resale_bulk_delete_rm(request):
    """RM Advanced Bulk Delete (Soft Delete) — scoped to the RM's own agricultural plots only."""
...
@require_POST
def agricultural_plot_resale_delete_rm(request, pk):
    """RM Soft Delete Agricultural Plot Resale Property"""
...
############# Views End for Resale Plot Agriculture Module for RM ######################################