...
############# Views start for tenant subscriptions ############################

def Subscriptions_Tenant(request):
    # 1. Retrieve BOTH possible session IDs from the browser
    user_id = request.session.get('User_id')
    admin_id = request.session.get('Admin_id') 
    logged_in_role = request.session.get('user_type')
    ...
    subscriptions_obj = Subscription_Details.objects.filter(plan_for="Tenants/Buyers")
    ...
    return render(request, "tenant_panel/Subscription/subscription.html", context)


############ Views end for tenant subscriptions ##################################


############ Views start for buy plan for tenant ##############################

def Buy_Plan_Tenant(request,id):
    # 1. Retrieve BOTH possible session IDs from the browser
    user_id = request.session.get('User_id')
    admin_id = request.session.get('Admin_id') 
    logged_in_role = request.session.get('user_type')
    ...
    return render(request, "tenant_panel/Subscription/buy_package.html", context)


############ Views end for buy plan for tenant ##############################