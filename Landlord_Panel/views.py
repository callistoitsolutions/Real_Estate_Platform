...
######### Views start for resale residential property view #######################

def residential_resale_landlord_view(request,id):
    # 1. Retrieve BOTH possible session IDs from the browser
    user_id = request.session.get('User_id')
    admin_id = request.session.get('Admin_id') 
    logged_in_role = request.session.get('user_type')
    ...
    return render(request, "landlord/Forms/Resale/residential_resale_view.html", context)

############### Views end for resale residential property view #######################


############ Views start for resale residential property edit ###################

def residential_resale_landlord_edit(request,id):
    # 1. Retrieve BOTH possible session IDs from the browser
    user_id = request.session.get('User_id')
    admin_id = request.session.get('Admin_id') 
    logged_in_role = request.session.get('user_type')
    ...
    return render(request, "landlord/Forms/Resale/residential_resale_edit.html", context)
...
########### Views start for update property form commercial ######################

def commercial_resale_landlord_update(request,id):
    ...
    return render(request,'landlord/Forms/Resale/commercial_resale_edit.html',context)


########## Views end for update property form commercial #######################


######### Views start for view property form commercial ##########################

def commercial_resale_landlord_view(request,id):
    ...
    is_valid_rm = (session_user_id and logged_in_role == "Landlord")
    ...
    return render(request, 'landlord/Forms/Resale/commercial_resale_view.html', context)

######## Views end for view property form commercial #########################
...
########### Views start for update residential plot from landlord ##############

def plot_resale_res_landlord_update(request,id):
    ...
    return render(request,"landlord/Forms/Resale/Plots/update_resi_plots.html",context)

############# Views end for update residential plot from landlord ####################


############ Views start for view residential plot from landlord ################

def plot_resale_res_landlord_view(request,id):
    ...
    return render(request,"landlord/Forms/Resale/Plots/view_resi_plots.html",context)

####### Views end for view residential plot from landlord #######################
...
############# Views start for update commercial plot for landlord ###################

def plot_resale_comm_landlord_update(request,id):
    ...
    # 1. Retrieve identity from browser session
        ...
        return render(request,"landlord/Forms/Resale/Plots/update_comm_plots.html",context)

############ Views end for update commercial plot from landlord #####################


############# Views start for view commercial plot from landlord ################

def plot_resale_comm_landlord_view(request,id):
    ...
    return render(request,"landlord/Forms/Resale/Plots/view_comm_plots.html",context)

############## Views end for view commercial plot from landlord ##################
...
############ Views start for update industrial resale property form ################

def industry_resale_landlord_update(request,id):
    ...
    return render(request, 'landlord/Forms/Resale/industrial_edit.html', context)

############## Views end for update industrial resale property form #####################


########## Views start for view industrial resale property form ##############

def industry_resale_landlord_view(request,id):
    ...
    return render(request, 'landlord/Forms/Resale/industrial_view.html', context)

########### Views end for view industrial resale property form ################


############ Views start for agricultural property list #################

def agriculture_resale_landlord_list(request):
    ...
    return render(request, 'landlord/Reports/Resale/agricultural_list.html', context)

############ Views end for agricutural property list #####################
...
def agriculture_resale_landlord_update(request,id):
    ...
    return render(request, 'landlord/Forms/Resale/edit_agricultural_resale.html', context)

############# Views end for update agricutural resale property form ####################


########### Views start for view agricultural resale property form ################

def agriculture_resale_landlord_view(request,id):
...
############# Views Start for Residential Rental Upload & Download Excel File Landlord #######################
...
# =====================================================================
# DOWNLOAD TEMPLATE (LANDLORD)
# =====================================================================

def download_residential_template_landlord(request):
    """Download the upload template for Landlord users. Listed By
    identity columns (name/email/contact/role) are MANDATORY and must
    exactly match your own registered Landlord profile — a row that's
    ...
    # ---- SECURITY CHECK (landlord pattern) ----
    user_id = request.session.get('User_id')
    admin_id = request.session.get('Admin_id')
    logged_in_role = request.session.get('user_type')
        ...
        for field, label, required in fields:
            ...
            if field in ("listed_by_name", "listed_by_email", "listed_by_contact", "listed_by_role"):
                ws.cell(row=2, column=col).comment = Comment(
                    "MANDATORY. Must exactly match your own registered Landlord\n"
    ...
    ws.protection.password = "estateflow2026"
...
# =====================================================================
# IMPORT (LANDLORD) — explicit identity MATCH required, else skip + alert
# =====================================================================

@csrf_exempt
@require_POST
def import_residential_excel_landlord(request):
    ...
    # ---- 1. SECURITY CHECK (landlord pattern, exactly as provided) ----
    user_id = request.session.get('User_id')
    admin_id = request.session.get('Admin_id')
    logged_in_role = request.session.get('user_type')
    ...
    # ---- 3. Data Fetching: the Landlord whose listings this upload belongs to ----
    try:
        landlord_obj = User_Details.objects.get(id=dashboard_user_id)
    except User_Details.DoesNotExist:
        return JsonResponse({
            "status": "error",
            "message": "Could not find the Landlord profile for this session.",
        }, status=400)
    ...
    if is_valid_admin and admin_obj:
        ...
        uploader_name = landlord_obj.user_name
        uploader_email = landlord_obj.user_email
        uploader_contact = landlord_obj.user_phone
        uploader_role = "User"
        user_identity = uploader_email or uploader_name

    # Identity every row's Listed By fields must MATCH — always the Landlord
    # (landlord_obj), never the impersonating admin.
    session_identity = {
        'id': landlord_obj.user_id,
        'name': landlord_obj.user_name,
        'email': landlord_obj.user_email,
        'contact': landlord_obj.user_phone,
        'role': landlord_obj.user_role,
    }
    ...
    for row_idx, row in enumerate(ws.iter_rows(min_row=data_start_row, values_only=True), start=data_start_row):
        ...
        # ---- Identity check: use _identity_matches_session, NOT
        # _identity_conflicts_with_session (that's a different boolean
        # sense and caused the NameError bug in the Agency file). ----
        if _identity_conflicts_with_session(obj_data, session_identity):
            ...
            if not any([typed_name, typed_email, typed_contact, typed_role]):
                ...
                reason = f"Listed By '{identity}' does not match your logged-in Landlord profile."
...
############# Views END for Residential Rental Upload & Download Excel File Lanlord #######################




############# Views start for rental Listing Module for landlord #########################
...
def residential_add_landlord(request):
    # 1. Retrieve identity from browser session
    user_id = request.session.get('User_id')
    admin_id = request.session.get('Admin_id') 
    logged_in_role = request.session.get('user_type')
 ...
 # ---------- AMENITIES & FACILITIES ----------
          ...
          # ---------- PROPERTY VIDEO (UPLOAD, RM LINK, OR AUTO SLIDESHOW) ----------
            ...
            return redirect('residential_landlord_list')
    ...
    return render(request, 'landlord/Forms/Rental/residential.html', {
        'user_obj': user_obj,
        'user_role': user_role,
...
def residential_landlord_view(request, pk):
    ...
    return render(request, 'landlord/Reports/Rental/rental_residential_detail.html', context)
...
def residential_landlord_edit(request, pk):
    ...
    if request.method == 'POST':
        try:
            ...
            return JsonResponse({
                ...
                'redirect_url': reverse('residential_landlord_list')
            })
    ...
    return render(request, 'landlord/Forms/Rental/residential_edit.html', context)
...
# Approval status breakdown
    ...
    return render(
        request,
        'landlord/Reports/Rental/residential_list.html',
        context
    )
...
@require_POST
def rental_residential_delete_landlord(request, pk):
    """Landlord Soft Delete Property (supports Admin impersonation)"""
    ...
    try:
        ...
        # Build identity string — note if this was an admin acting on the landlord's behalf
        if is_valid_admin:
            admin_obj = Admin_Login.objects.filter(id=admin_id).first()
            ...
            user_role = "Admin (Impersonating Landlord)"
        else:
            user_identity = (
            ...
            user_role = user_obj.user_role or "Landlord"
        ...
        # ======================================
        # 6. Security Check — landlord can delete only own property
        # ======================================
...
@require_POST
def rental_bulk_delete_landlord(request):
    """Landlord Advanced Bulk Delete (Soft Delete) — scoped to landlord's own properties, supports Admin impersonation."""
    ...
    try:
        ...
        # ======================================
        # 6. Base Queryset — SCOPED TO THIS LANDLORD ONLY
        # ======================================
...
######### Views start for subscription for boost property landlord #############

def Boost_Property_Landlord(request):
    ...
    subscriptions_obj = Subscription_Details.objects.filter(plan_for="Landlords")
    ...
    return render(request, "landlord/Property_Boost/boost_property.html", context)


############ Views end for subscription for boost property landlord ###############


########## Views start for buy subscription plan for landlord ###################

def Buy_Plan_Landlord(request,id):
    ...
    return render(request, "landlord/Property_Boost/buy_package.html", context)


########## Views start for ajax for buy subscription #####################

@csrf_exempt
def Buy_Subscription_Ajax(request):
    data = request.POST.dict()

    data['fk_user'] = User_Details.objects.get(user_id=data['fk_user'])       
    data['fk_subscription'] = Subscription_Details.objects.get(id=data['fk_subscription']) 

    if data.get('id') == "":
        data.pop("id", None)
        
        data['payment_ss'] = request.FILES.get('payment_ss')        
        data['purchase_date'] = datetime.today()
        data['purchase_time'] = datetime.now()
        
        Subscription_Purchase_Details.objects.create(**data)

        return JsonResponse({"status":"1", "msg" : f"Subscription puchased successfully it takes 24 to 48 hours of approval time."})

    # UPDATE MODE
    else:
        try:
            payment = Subscription_Purchase_Details.objects.get(id=data['id'])
        except Subscription_Purchase_Details.DoesNotExist:
            return JsonResponse({'status': '0', 'msg': 'Payment Details not found'})
       
        data['payment_ss'] = request.FILES.get('payment_ss')

        if request.FILES.get('payment_ss'):
            data['payment_ss'] = request.FILES.get('payment_ss')
        else:
            data.pop('payment_ss', None)
...
def commercial_rental_add_landlord(request):
    ...
    return render(request, 'landlord/Forms/Rental/commercial.html'
   , {
        'user_obj': user_obj,
...
def commercial_landlord_edit(request, pk):
    ...
    return render(request, 'landlord/Forms/Rental/commercial_edit.html', context)
...
def commercial_view_landlord(request, pk):
    user_id = request.session.get('User_id')
    admin_id = request.session.get('Admin_id') 
    logged_in_role = request.session.get('user_type')
    ...
    return render(request, 'landlord/Reports/Rental/commercial_detail.html', {
...
def export_commercial_rent_landlord(request):
    user_id = request.session.get('User_id')
    admin_id = request.session.get('Admin_id') 
    logged_in_role = request.session.get('user_type')
...
@require_POST
def commercial_delete_landlord(request, pk):
    """Landlord Soft Delete Commercial Property (supports Admin impersonation)"""
    ...
    try:
        ...
        if is_valid_admin:
            ...
            user_identity = (
                f"[Impersonated] Acting Admin: {admin_label} | "
                f"On behalf of User ID: {landlord_obj.user_id} | "
                f"Name: {landlord_obj.user_name} | "
                f"Email: {landlord_obj.user_email} | "
                f"Phone: {landlord_obj.user_phone} | "
                f"Role: {landlord_obj.user_role}"
            )
            user_role = "Admin (Impersonating Landlord)"
        else:
            user_identity = (
                f"User ID: {landlord_obj.user_id} | "
                f"Name: {landlord_obj.user_name} | "
                f"Email: {landlord_obj.user_email} | "
                f"Phone: {landlord_obj.user_phone} | "
                f"Role: {landlord_obj.user_role}"
            )
            user_role = landlord_obj.user_role or "Landlord"
        ...
        # 6. Security Check — landlord can delete only own property
        # ======================================
        if str(prop.listed_by_id) != str(landlord_obj.user_id):
            return JsonResponse({
                'status': 'error',
                'message': 'You can delete only your own properties.'
            })
...
@require_POST
def commercial_bulk_delete_landlord(request):
    """Landlord Advanced Bulk Delete (Soft Delete) — scoped to landlord's own commercial properties, supports Admin impersonation."""
    ...
    try:
        ...
        properties = CommercialRentalProperty.objects.filter(
            is_deleted=False,
            listed_by_id=landlord_obj.user_id
        )
...
############### Views end for commercial rental listing for landlord #############################




############# Views start for Rental pg Module for Landlord ###################################################
...
def pg_landlord_list(request):
    ...
    return render(request, 'landlord/Reports/Rental/pg_list.html', {
...
def add_pg_landlord(request):
    ...
    try:
        ...
        uploader_role = getattr(user_obj, 'user_role', 'User') if user_obj else 'Landlord'
        uploader_id = f"USER_{user_id}"
...
def pg_landlord_edit(request, pk):
    property_id = pk
    user_id = request.session.get('User_id')
    admin_id = request.session.get('Admin_id') 
    logged_in_role = request.session.get('user_type')
    ...
    if request.method == "GET":
        ...
        return render(request, 'landlord/Forms/Rental/pg_edit.html', context)
    ...
    try:
        ...
        return JsonResponse({
            ...
            "redirect_url": reverse('pg_landlord_list')
        })
...
def export_pg_coliving_landlord(request):
...
def pg_coliving_view_landlord(request, pk):
    ...
    return render(request, "landlord/Reports/Rental/pg_coliving_view.html", context)



@require_POST
def pg_coliving_delete_landlord(request, pk):
    """Landlord Soft Delete PG/Coliving Property (supports Admin impersonation)"""
    ...
    try:
        ...
        # 6. Security Check — landlord can delete only own property
        # ======================================
        if str(pg.listed_by_id) != str(landlord_obj.user_id):
            return JsonResponse({
                'status': 'error',
                'message': 'You can delete only your own properties.'
            })
...
@require_POST
def pg_bulk_delete_landlord(request):
    """Landlord Advanced Bulk Delete (Soft Delete) — scoped to landlord's own PG/Coliving properties, supports Admin impersonation."""
    ...
    try:
        ...
        try:
            ...
            return JsonResponse({'status': 'error', 'message': 'Landlord not found.'}, status=404)
...
############# Views END for Rental pg Module for Landlord ###################################################





############# Views Start for Resale Plot Industrial Module for Landlord ######################################



def industrial_plot_resale_landlord(request):
    user_id = request.session.get('User_id')
    admin_id = request.session.get('Admin_id') 
    logged_in_role = request.session.get('user_type')
    ...
    return render(request, "landlord/Forms/Resale_Plot/industrial_plot_resale.html", context)
...
def industrial_plot_resale_add_landlord(request):
    ...
    return render(request, 'landlord/Reports/Resale_Plot/industrial_plot_resale_list.html', {
...
def industrial_plot_resale_edit_landlord(request, pk):
    ...
    "redirect_url": reverse('industrial_plot_resale_list_landlord')
})
        except Exception as e:
            print("ERROR DETECTED:", str(e))
            return JsonResponse({
    ...
    return render(request, 'landlord/Reports/Resale_Plot/industrial_plot_resale_edit.html', {
...
def industrial_plot_resale_view_landlord(request, pk):
    user_id = request.session.get('User_id')
    admin_id = request.session.get('Admin_id') 
    logged_in_role = request.session.get('user_type')
    ...
    return render(request, 'landlord/Reports/Resale_Plot/industrial_plot_resale_view.html', {
        'user_obj': user_obj,
...
def industrial_plot_resale_list_landlord(request):
    ...
    return render(request, "landlord/Reports/Resale_Plot/industrial_plot_resale_list.html", context)




@require_POST
def industrial_plot_resale_delete_landlord(request, pk):
    """Landlord Soft Delete Industrial Plot Resale Property (supports Admin impersonation)"""
    ...
    try:
        ...
        # 6. Security Check — landlord can delete only own property
        # ======================================
        if str(plot.listed_by_id) != str(landlord_obj.user_id):
            return JsonResponse({
                'status': 'error',
                'message': 'You can delete only your own properties.'
            })
        ...
        IndustrialPlotResaleActivityLog.objects.create(
            ...
            action_type="DELETE",
            property_id=plot.id,
            action_payload=json.dumps({"reason": "Manual delete via Landlord list page"}),
            status="SUCCESS",
        )
...
@require_POST
def industrial_plot_resale_bulk_delete_landlord(request):
    """Landlord Advanced Bulk Delete (Soft Delete) — scoped to landlord's own industrial plots, supports Admin impersonation."""
    ...
    try:
        ...
        if delete_type == "current_page":
            ...
            pass  # qs already scoped to this landlord's non-deleted rows
...
def export_industrial_plot_resale_landlord(request):
...
def agricultural_plot_resale_landlord(request):
    user_id = request.session.get('User_id')
    admin_id = request.session.get('Admin_id') 
    logged_in_role = request.session.get('user_type')
    ...
    return render(request, "landlord/Forms/Resale_Plot/agricultural_plot_resale.html", context)
...
def agricultural_plot_resale_add_landlord(request):
    user_id = request.session.get('User_id')
    admin_id = request.session.get('Admin_id') 
    logged_in_role = request.session.get('user_type')
    ...
    return render(request, 'landlord/Reports/Resale_Plot/agricultural_plot_resale_list.html', {
...
def agricultural_plot_resale_edit_landlord(request, pk):
    user_id = request.session.get('User_id')
    admin_id = request.session.get('Admin_id') 
    logged_in_role = request.session.get('user_type')
    ...
    if request.method == 'POST':
        try:
            ...
            return JsonResponse({
                ...
                'redirect_url': reverse('agricultural_plot_resale_list_landlord')
            })
    ...
    return render(request, 'landlord/Reports/Resale_Plot/agricultural_plot_resale_edit.html', {
...
def export_agricultural_plot_resale_landlord(request):
...
def agricultural_plot_resale_view_landlord(request, pk):
    ...
    return render(request, 'landlord/Reports/Resale_Plot/agricultural_plot_resale_view.html', {
        'user_obj': user_obj,
...
def agricultural_plot_resale_list_landlord(request):
    user_id = request.session.get('User_id')
    admin_id = request.session.get('Admin_id') 
    logged_in_role = request.session.get('user_type')
    ...
    return render(request, "landlord/Reports/Resale_Plot/agricultural_plot_resale_list.html", context)




@require_POST
def agricultural_plot_resale_delete_landlord(request, pk):
    """Landlord Soft Delete Agricultural Plot Resale Property (supports Admin impersonation)"""
...
@require_POST
def agricultural_plot_resale_bulk_delete_landlord(request):
    """Landlord Advanced Bulk Delete (Soft Delete) — scoped to landlord's own agricultural plots, supports Admin impersonation."""
